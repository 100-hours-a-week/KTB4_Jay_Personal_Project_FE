...
- 개인적인 고민과 개발 경험을 주제로 사용자가 글을 작성하고, 댓글, 좋아요, 신고, 실시간 채팅으로 소통할 수 있는 커뮤니티 서비스의 프론트엔드입니다.
...
| 구분 | 링크 |
|---|---|
| Backend | [github_be_link](https://github.com/100-hours-a-week/KTB4_Jay_Personal_Project_BE) |
| Frontend | [github_fe_link](https://github.com/100-hours-a-week/KTB4_Jay_Personal_Project_FE) |
| 시연 영상 | [google_drive_link](https://drive.google.com/file/d/1CjIGSUEz2xKyWmeRw4WlikOgNkLoRqIr/view?usp=sharing) |
...
| 기술 | 사용 이유 |
|---|---|
| React | Vanilla JavaScript에서 화면, 이벤트, API 호출이 섞이던 구조를 페이지/컴포넌트 단위로 나누고, 로그인 상태와 목록 상태를 React state 기준으로 관리하기 위해 사용했습니다. |
| Vite | React 개발 서버와 production build를 빠르게 구성하고, `.env.production`의 `/api` 같은 배포용 API base URL을 빌드 시점에 명확히 주입하기 위해 사용했습니다. |
| Fetch API | 별도 HTTP client 의존성을 추가하지 않고 브라우저 기본 API로 REST 요청을 처리하기 위해 사용했습니다. 공통 `api/client.js`에서 Authorization 헤더, JSON 파싱, 에러 메시지 변환을 한 번에 처리했습니다. |
| STOMP.js | WebSocket을 직접 문자열 프로토콜로 다루지 않고, 연결/구독/발행 흐름을 명확하게 표현하기 위해 사용했습니다. 게시글별 topic 구독과 JWT 연결 헤더 전달을 프론트에서 관리했습니다. |
| contenteditable Markdown Editor | 코드리뷰 게시글이라는 서비스 컨셉에 맞게 코드블록과 이미지를 일반 textarea보다 자연스럽게 작성하도록 만들기 위해 사용했습니다. |
| Nginx | React 정적 파일을 서빙하고 `/api/*`, `/ws` 요청을 백엔드 컨테이너로 프록시하기 위해 사용했습니다. 프론트와 백엔드를 같은 EC2 안의 Docker Compose 네트워크로 연결했습니다. |
| Docker / GitHub Actions | 로컬 `dist` 복사 방식 대신 이미지 기반으로 같은 빌드 절차를 반복하고, EC2에서는 `docker compose pull`, `up -d`로 배포를 단순화하기 위해 사용했습니다. |
...
- 게시글 목록, 인기글 목록, 채팅 메시지 조회처럼 비로그인 접근이 가능한 기능은 백엔드에서 `currentUserId`가 null일 수 있도록 고려했습니다.
...
### 5. 좋아요/좋아요 취소 후 전체 목록을 다시 불러오는 문제
...
### 7. 채팅 연결에서 인증과 메시지 흐름을 분리해야 하는 문제
...
### 8. Docker 배포에서 로컬 빌드 산출물과 운영 빌드가 섞이는 문제
...
### 10. 코드리뷰용 게시글에서 코드와 이미지를 자연스럽게 작성하기 어려운 문제
